...
import { coFoundersData } from "./../../data/CofounderData";

function CofounderCard({ CoFounder }) {
  return (
    <div className="bg-secondary-gray/31 pl-20 p-4 text-xs relative mt-4 container max-h-[195px] group ml-20 rounded-2xl max-w-[350px]">
      <div className="absolute -left-18 top-10  transition-transform transform duration-500">
        <div className="relative">
          <img
            src={CoFounder.image}
            alt={CoFounder.name.toLowerCase().replace(/\s+/g, "-")}
          />
          ...
          <div className="text-sm">
            ...
            <div className="flex justify-between">
              <div className="text-[10px] font-medium">
                <p className="uppercase">{CoFounder.position}</p>
                <p className="text-gray-400">{CoFounder.company}</p>
              </div>
...
// Container component to render multiple CoFounder cards
function CofounderSection({ domain, search }) {
  let filteredCoFounders = coFoundersData.filter((data) =>
    domain === "All" ? true : data.domain.includes(domain)
  );
  filteredCoFounders = filteredCoFounders.filter((data) =>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2  gap-15   justify-items-end p-8 overflow-y-auto h-[750px] ">
      {filteredCoFounders.length == 0 ? (
        <div className="sticky top-0 flex items-center justify-center h-[200px] text-gray-400 font-semibold text-4xl w-full col-span-2 capitalize bg-white">
          No CoFounders available with {domain} Domain
        </div>
      ) : (
        filteredCoFounders.map((CoFounder) => (
          <CofounderCard key={CoFounder.id} CoFounder={CoFounder} />
        ))
      )}
    </div>
  );
